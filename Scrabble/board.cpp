...
std::vector<unsigned int> Board::caseQueOnPeutRemplir(){
  std::vector<unsigned int> tableauContenantLesCasesRemplisables;
  if(spots[112].letter == 0) { //begining of game 
    tableauContenantLesCasesRemplisables.push_back(112);
  } else {
    for(int i=0; i<225; i++){
      if(spots[i].letter != 0){     
        if(i+1 < 225 && spots[i+1].letter == 0) {
          if((i+1)%15 != 0) {
            tableauContenantLesCasesRemplisables.push_back(i+1);
          }
        }
        if(i+15 < 225 && spots[i+15].letter == 0) {
          tableauContenantLesCasesRemplisables.push_back(i+15);
        }
        if(i-1 >= 0 && spots[i-1].letter == 0) {
          if(14-((i-1)-(((i-1)/15)*15)) != 0) {
          tableauContenantLesCasesRemplisables.push_back(i-1);
         }
        }
        if(i-15 >= 0 && spots[i-15].letter == 0) {
          tableauContenantLesCasesRemplisables.push_back(i-15);
        }     
      }
    }
   }
  return tableauContenantLesCasesRemplisables;
}

void Board::placerUnMotSurLePlateau(std::stringstream & ss, Etat *e) {
  std::string mot = "";
  for(int l=0; l<e->chaineDeLettresDejaParcouruPourArriveACetEtat.size(); l++) {
  ...
  for(int i=0; i<mot.size(); i++) {
    spots[e->numerosDesCasesSurLesquelsSontLesLettresDeLaChaine[i]].letter = mot[i];
  }

  std::string ligne = "";
  for(unsigned int j=0; j<15; j++) {
    for(unsigned int k=0; k<15; k++) {
      if(spots[(15*j)+k].letter == 0) {
        ligne+='.';
      } else {
        ligne+=spots[(15*j)+k].letter;
      }
    }
    ss << ligne << std::endl;
    ligne = "";
